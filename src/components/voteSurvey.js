...
export default function Example({ user }) {
  ...
  const [abgeschicktCompletedAt, setAbgeschicktCompletedAt] =
    useState("eben gerade");
  ...
  useEffect(() => {
    ...
    setTimeout(function () {
      ...
      updateDoc(docRefy, data).then(() => {
        console.log("updated", docRefy, data);
      });

      const colRef2 = collection(docRef, "questions");
      console.log("here 2");

      titlesetter().then((res) => {
        setTitle(res);
      });
  ...
  function abschicken() {
    ...
    questionAndAnswerArray.forEach((item) => {
      ...
      setDoc(doc(colRefVotes, user.uid), { vote: "Vote" })
        .then((docRef) => {
  ...
  async function setVote(colRef, questionId, answerId, answerText) {
    ...
    warten().then((check) => {
      ...
      const versuch = setDoc(doc(colRef, user.uid), {
        questionId,
        answerId,
        answerText,
      })
        .then(() => {
          const gesIndex = questionAndAnswerArray.findIndex(
    ...
    <StyledBox>
      ...
      <h1>
        {times && votingCompleted && !abgeschickt && (
          <>
            "Alle Fragen wurden beantwortet, aber noch nicht abgeschickt."{" "}
            <button onClick={abschicken()}>Abschicken</button>
          </>
        )}
      </h1>
      {times && isLoading && <p>Lädt...</p>}
      {times && abgeschickt && !isLoading && (
        <h1>Umfrage bereits abgeschickt am : {abgeschicktCompletedAt}.</h1>
      )}
          ...
          {questionArray.map((item, index) => {
            return (
              <AC
                key={item.id}
                setVote={setVote}
                questionDocRef={questionDocRefs[index]}
                questionId={item.id}
                creatorId={creatorId}
                surveyId={surveyId}
                questionText={item.text}
